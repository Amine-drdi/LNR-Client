  ...
  const handleUpdateDevis = async () => {
    try {
      const response = await axios.put(
        `http://51.83.69.195:5000/api/deviscomm/${selectedDevis._id}`,
        {
          heure: selectedDevis.heure,
          devisDate: selectedDevis.devisDate,
          effetDate: selectedDevis.effetDate,
          commentaireAgent: selectedDevis.commentaireAgent,
          compagnie: selectedDevis.compagnie,
          cotisation: selectedDevis.cotisation
        }
      );
    ...
    <div className="max-w-full mx-auto p-6 bg-blue-gray-50 rounded-lg shadow-lg">
      ...
      >
            ...
            <p className='text-left'><strong>Apporteur d'affaire:</strong> {selectedDevis.apporteurAffaire}</p>
             {/* Champ pour modifier l'heure' */}
             <div className="mb-4">
            <label className="block text-base font-bold text-black text-left">Heure:</label>
            <input
            type="time"
            value={selectedDevis.heure || ''}
             onChange={(e) =>
            setSelectedDevis({ ...selectedDevis, heure: e.target.value })
            }
          ...
          {/* Champ pour modifier la date */}
             <div className="mb-4">
        <label className="block text-base font-bold text-black text-left">Date du devis:</label>
        <input
          type="date"
          value={selectedDevis.devisDate || ''}
          onChange={(e) =>
            setSelectedDevis({ ...selectedDevis, devisDate: e.target.value })
          }
          className="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm"
        />
      </div>
            ...
            <div className="mb-4">
        <label className="block text-base font-bold text-black text-left">Date d'effet:</label>
        <input
          type="date"
          value={selectedDevis.effetDate || ''}
          onChange={(e) =>
            setSelectedDevis({ ...selectedDevis, effetDate: e.target.value })
          }
          className="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm"
        />
      </div>
      ...
      <div className="mb-4">
        <label className="block text-base font-bold text-black text-left">Compagnie:</label>
        <input
          type="text"
          value={selectedDevis.compagnie || ''}
          onChange={(e) =>
            setSelectedDevis({ ...selectedDevis, compagnie: e.target.value })
          }
          className="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm"
        />
      </div>
      <div className="mb-4">
        <label className="block text-base font-bold text-black text-left">Montant VP:</label>
        <input
          type="number"
          value={selectedDevis.cotisation || ''}
          onChange={(e) =>
            setSelectedDevis({ ...selectedDevis, cotisation: e.target.value })
          }
          className="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm"
        />
      </div>
           
             {/* Champ pour modifier le commentaireAgent */}
      <div className="mb-4">
        <label className="block text-base font-bold text-black text-left">Commentaire Agent:</label>
        <textarea
          value={selectedDevis.commentaireAgent || ''}
          onChange={(e) =>
            setSelectedDevis({ ...selectedDevis, commentaireAgent: e.target.value })
          }
          className="w-full h-24 px-4 py-2 border border-gray-300 rounded-md shadow-sm"
        />
      </div>
            ...
            <div className="mt-4 flex justify-end">
              ...
              <button onClick={handleUpdateDevis} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-700 transition">
                Modifier
              </button>
              </div>
            </div>
